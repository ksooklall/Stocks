...
class rnn_network():
    ...
    def build_embedding_matrix(self, ticker):
        with tf.variable_scope('embeddings'):
            embedding_matrix = tf.random_uniform([self.stock_count, self.embedding_size], minval=-1.0, maxval=1.0)
            tiled_embeddings = tf.tile(ticker, [1, self.num_steps], name='tiled')
            label_embeddings = tf.nn.embedding_lookup(embedding_matrix, tiled_embeddings, name='emb_labels')
        inputs_with_embeddings = tf.concat(self.inputs, label_embeddings, axis=2)
    ...
    def build_output(self, lstm_size, output):
        ...
        tf.summary.histogram('output/add:0', prediction)
        return prediction
    ...
    def train(self, lstm_size, layers, resume=False, batch_size=32, kp=0.2, epochs=10, lr=0.001, verbose=5):
        ...
        with tf.Session() as sess:
            merged, writer = self.merge_summaries(sess, 'summaries/1')
            if resume:
                checkpoint = tf.train.latest_checkpoint('checkpoints/1')
